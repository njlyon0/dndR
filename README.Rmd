...
The first such function is `xp_total()` and it identifies the total experience points of all creatures given a party level (average of PCs level in party) and a desired difficulty level (easy, medium, hard, or deadly). GMs can use this value to "buy" creatures/monsters based on their XP until the pool is exhausted to ensure their encounters are the desired level of difficulty for the specified party.

```{r xp_total_fxn}
dndR::xp_total(party_level = 3, difficulty = 'medium')
```

Here is a comparison of the total XP for an easy encounter recommended in the DMG versus the XP calcualted by `dndR::xp_total()` for the same difficulty of encounter.
...
ggplot2::ggplot(xp_df, aes(x = pc_level, y = xp,
  ...
  geom_point(size = 2, pch = rep(x = c(17, 19), times = 20)) +
