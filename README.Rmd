...
Creating your own creatures can be a great way to add flavor to an encounter! Dungeon Master's Guide provides a table (see p. 274) that gives the core vital statistics for creatures based on their Challenge Rating (CR) but this table can be cumbersome to compare to Experience Points (you know, the things used to determine how hard an encounter will be for your party?). To aid with this, I've created `monster_quick()` which accepts either CR or XP and returns a quick summary of what statistics the DMG recommends for a creature of the CR/XP value.You can specify either XP or CR and it can convert between the two internally as needed. Note that if you specify both XP and CR, the function will ignore XP and proceed with CR alone.

```{r monster_quick_fxn1}
