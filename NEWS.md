...
- `encounter_creator()` supports a `try` argument that defines how many attempts are made to maximize encounter XP while remaining beneath the allowable XP threshold.
- `encounter_creator()` supports a `max_creatures` parameter that allows users to specify the maximum number of creatures they want to include in a given encounter.
...
- `creature_list()` identifies Dungeons & Dragons creatures based on user specifications of various criteria (e.g., creature type, experience point value, size, etc.).
...
- Fixed non-ASCII apostrophes in spell names (causes failure to find spell with `spell_text` and `spell_list`).
...
- `roll()` description clarified and expanded.
